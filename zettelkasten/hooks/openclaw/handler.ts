...
- Analysis or summaries of articles, blog posts, or papers -> \`$\{ZK_DIR\}/Literature/\`
- Things learned about technologies or tools -> \`$\{ZK_DIR\}/Literature/\`
- Ideas, reflections, and best practices -> \`$\{ZK_DIR\}/Permanent/\`
- Before creating a new note, search for related notes and add \`[[links]]\` in the body

Basics when creating notes:
- Use the filename format \`YYYY-MM-DD HH.mm.ss.md\`
- Add \`type: Literature\` or \`type: Permanent\` to the frontmatter
...
const handler: HookHandler = async (event) => {
  ...
  const cleanedBootstrapFiles = event.context.bootstrapFiles.filter(
    (file, index, files) =>
      !isInjectedReminderFile(file) ||
      files.findIndex((candidate) => isInjectedReminderFile(candidate)) ===
        index,
  );
  ...
  const existingIndex = cleanedBootstrapFiles.findIndex((file) =>
    isInjectedReminderFile(file),
  );
  if (existingIndex === -1) {

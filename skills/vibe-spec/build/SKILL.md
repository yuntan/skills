...
2. **Establish a clean baseline.** Run `git status --porcelain`. If there are uncommitted changes outside the expected planning artifacts (`spec/*`, `.tasks/plan.md`, `.tasks/todo.md`), stop and ask the user to commit, stash, or confirm how to handle them. Autonomous per-task commits must not absorb unrelated local work, or the clean-rollback guarantee breaks.
...
5. **Execute every task in dependency order.** Use each task's declared dependencies; if they aren't explicit, execute in the order the plan lists them. Delegate each task's implementation to a **subagent running the Sonnet model** (via the Agent tool, `model: "sonnet"`) — pass it the task, its acceptance criteria, and the RED → GREEN → regression → build loop to run. Stage only the files that task touched plus its task-status update — never `git add -A` blindly — and make one commit per task so any point is a clean rollback.
